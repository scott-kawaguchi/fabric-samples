...
async function main() {
	try {
		...
		try {
			...
			console.log('\n--> Submit Transaction: CreateVote, creates new vote with ID, firstName, lastName, driversLicenseId, imageUrls, videoUrls, soundUrls, vote, gatewayId, gatewayLocation');
			result = await contract.submitTransaction('CreateVote', 'user4', 'James', 'Harden', 'B3849234', 'http://securevote.com/bucketid/image', 'http://securevote.com/bucketid/myvid', 'http://securevote.com/bucketid/mysoundbite', 'George', 'gateway2', 'Washington coordinates: 47.751076, -120.740135');
			...
			console.log('\n--> Evaluate Transaction: ReadVote, function returns an asset with a given assetID');
			result = await contract.evaluateTransaction('ReadVote', 'user4');
			console.log(`*** Result: ${prettyJSONString(result.toString())}`);

			console.log('\n--> Evaluate Transaction: VoteExists, function returns "true" if an user voted given userID exist');
			result = await contract.evaluateTransaction('VoteExists', 'user1');
			console.log(`*** Result: ${prettyJSONString(result.toString())}`);
